...
test('OrarioDoc unit tests', async ({ page }) => {
  ...
  const writeDiagnostics = async (stats) => {
    await page.screenshot({ path: path.join(resultsDir, 'unit-test-screenshot.png'), fullPage: true }).catch(() => {});
    fs.writeFileSync(path.join(resultsDir, 'unit-page-console.log'), logs.join('\n'), 'utf8');
    fs.writeFileSync(path.join(resultsDir, 'unit-test-result.html'), await page.content(), 'utf8');
    if (stats) fs.writeFileSync(path.join(resultsDir, 'unit-test-results.json'), JSON.stringify(stats, null, 2), 'utf8');
  };

  await page.goto('/tests/unit-test-runner.html', { waitUntil: 'domcontentloaded' });
  ...
  try {
    ...
    await page.screenshot({ path: path.join(resultsDir, 'unit-test-failure-debug.png'), fullPage: true }).catch(() => {});
  ...
  console.log('Unit Test Results:', stats);
  
  // Write simplified results to test-results.json at root for CI compatibility
  ...
  fs.writeFileSync(path.join(process.cwd(), 'test-results.json'), JSON.stringify(simplifiedResults, null, 2), 'utf8');
  
  if (stats.failedItems && stats.failedItems.length) {
    console.error('=== FAILED UNIT TESTS START ===\n' + stats.failedItems.join('\n---\n') + '\n=== FAILED UNIT TESTS END ===');
    // Append failures to GitHub Actions summary for quick access in CI
    const summaryPath = process.env.GITHUB_STEP_SUMMARY;
    if (summaryPath) {
      try {
        const text = ['## Unit test failures', '', ...stats.failedItems.map((s, i) => `${i + 1}. ${s}`)].join('\n');
